...
LEFT JOIN `studied-zephyr-393513.hibob_trial.account_cycle` cycle ON arr.account_name = cycle.account_name
LEFT JOIN `studied-zephyr-393513.hibob_trial.account_ps` ps ON arr.account_name = ps.account_name
LEFT JOIN `studied-zephyr-393513.hibob_trial.ps_status` pss ON ps.ps_id = pss.ps_id
),
...
SELECT 
  ...
  arr_dec_2023.arr
FROM account_latest
JOIN arr_dec_2023 ON account_latest.account_name = arr_dec_2023.account_name
FULL OUTER JOIN account_last_approved ON account_latest.account_name = account_last_approved.account_name
